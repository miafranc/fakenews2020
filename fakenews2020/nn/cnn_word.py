...
class CNNWord(NNBase):
    ...
    def build_model(self):
        ...
        ilayers = []
        for filtersize, filters, poolsize in zip(filtersize_list, number_of_filters_per_filtersize, pool_length_list):
            conv = layers.Conv1D(filters, filtersize, input_shape=(self.text_length, len(self.word_map)))(emb_layer)
            activ = layers.Activation('relu')(conv)
            maxpool = layers.MaxPooling1D(poolsize)(activ)
            flat = layers.Flatten()(maxpool)
            ilayers.append(flat)
        layer = layers.concatenate(ilayers)
